def export(validate=False):
    ...
    for source_file in source_dir.iterdir():
        ...
        print(f"Processing {source_file.relative_to(print_relative_to)} -> {dest_file.relative_to(print_relative_to)}")
        ...
        if source_file.suffix == ".ipynb":
            ...
            code = [f"# Autogenerated! Edit {source_file.relative_to(print_relative_to)} instead {'VALIDATED' if validate else''}\n"]
            exported_count = 0
            ...
            if len(code) == 2:
                ...
                with open(dest_file, "r+t") as fd:
                    if mintils.have_same_conents(fd, code_joined):
                        ...
                        print(f"  {exported_count} cells ", end="")
                        if validate:
                            print(f"... validating ... ", end="", flush=True) # Can take a while to run so flush
                            ...
                            if res.returncode or res.stdout or res.stderr:
                                print(f"failed, exiting, {mintils.pop_string_timing_scope()}\nstdout: {bool(res.stdout)}, stderr: {bool(res.stderr)}, returncode: {res.returncode}")
                                print(res.stderr)
                                quit(-1)

                            dest_file_temp.unlink()

                        print(f"exported, {mintils.pop_string_timing_scope()}")
